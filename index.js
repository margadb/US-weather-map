...
Object.keys(state_info).forEach(function(key){
    ...
    //console.log(key);
    var urlLat = value.lat;
    var urlLng = value.lng;
    var cap = value.capital;
    var url = 'https://api.darksky.net/forecast/4ef8db52ec2d1f5ebb0dc4fbbe5aefa7/' + urlLat + ',' + urlLng;
    $.ajax({url:url, dataType:"jsonp"}).then(function(data){
    	
    	var temp = data.currently.temperature
    	console.log(key,cap,temp,data)
    	//$('#temp').text(data.currently.temperature)
    	if (temp <= '10.00'){
    		$('#'+key).css('fill',"blue");
    	}
    	else if (temp >= '11.00' && temp <= '30.00'){
    		$('#'+key).css('fill',"cyan");
    	}
    	else if (temp >= '31.00' && temp <= '50.00'){
    		$('#'+key).css('fill',"green");
    	}
    	else if (temp >= '51.00' && temp <= '80.00'){
    		$('#'+key).css('fill',"orange");
    	}
    	else{
    		$('#'+key).css('fill',"red");
